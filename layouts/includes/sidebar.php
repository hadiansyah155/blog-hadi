<div class="sidebar">
    <nav class="sidebar-nav">
        <ul class="nav">
            <li class="nav-item">
                <a class="nav-link" href="/11-rpl2/blog-hadi/admin">
                    <i class="nav-icon icon-speedometer"></i> Dashboard
                    <span class="badge badge-primary">Home</span>
                </a>
            </li>
            <li class="nav-title">Manajemen Blog</li>
            <li class="nav-item">
                <a class="nav-link" href="/11-rpl2/blog-hadi/admin/kategori">
                    <i class="nav-icon icon-drop"></i> Kategori</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="/11-rpl2/blog-hadi/admin/artikel">
                    <i class="nav-icon icon-pencil"></i> Artikel</a>
            </li>
            <li class="divider"></li>
        </ul>
    </nav>
    <button class="sidebar-minimizer brand-minimizer" type="button"></button>
</div>